...
`ableppt` 是一套面向金融场景的 **PowerPoint 自动化解决方案**。它负责把：
...
ableppt validate-job job_demo_company_a.json
ableppt render job_demo_company_a.json
ableppt validate-ppt output/样例公司A竞争壁垒分析.pptx --json
```

### Tushare -> SQLite 数据准备

```bash
python scripts/build_tushare_sqlite.py --output ableppt/data/tushare_market.sqlite
...
```bash
ableppt parse-template aim/aim00.pptx
ableppt generate aim/aim00.pptx config.json output/demo.pptx
```
...
ableppt parse-waterfall output/waterfall-demo.pptx

ableppt render-scatter scatter_demo.json output/scatter-demo.pptx
ableppt parse-scatter output/scatter-demo.pptx

ableppt render-bubble bubble_demo.json output/bubble-demo.pptx
ableppt parse-bubble output/bubble-demo.pptx

ableppt render-range-snapshot range_snapshot_demo.json output/range-snapshot-demo.pptx
ableppt parse-range-snapshot output/range-snapshot-demo.pptx

ableppt render-range-snapshot range_snapshot_sector_demo.json output/range-snapshot-sector-demo.pptx

ableppt render-family performance_compare_demo.json output/performance-compare-demo.pptx
ableppt render-family distribution_history_demo.json output/distribution-history-demo.pptx
ableppt render-family score_overlay_demo.json output/score-overlay-demo.pptx
ableppt render-family event_timeline_demo.json output/event-timeline-demo.pptx
ableppt render-family style_box_demo.json output/style-box-demo.pptx
ableppt render-family ranked_tile_matrix_demo.json output/ranked-tile-matrix-demo.pptx
ableppt render-family heatmap_matrix_demo.json output/heatmap-matrix-demo.pptx
ableppt render-family table_plus_chart_composite_demo.json output/table-plus-chart-composite-demo.pptx
ableppt render-family factor_attribution_panel_demo.json output/factor-attribution-panel-demo.pptx
ableppt render-family regime_table_panel_demo.json output/regime-table-panel-demo.pptx
ableppt render-family manager_timeline_profile_demo.json output/manager-timeline-profile-demo.pptx
ableppt render-family award_timeline_panel_demo.json output/award-timeline-panel-demo.pptx
ableppt render-family selection_timing_grid_demo.json output/selection-timing-grid-demo.pptx
ableppt render-family holding_detail_demo.json output/holding-detail-demo.pptx
ableppt render-family dual_chart_panel_demo.json output/dual-chart-panel-demo.pptx

ableppt render job_research_shell_demo.json
ableppt render job_dashboard_shell_demo.json
ableppt render job_factsheet_shell_demo.json
ableppt render job_summary_shell_demo.json
ableppt render job_section_cover_demo.json
ableppt render job_chapter_divider_demo.json
ableppt render job_profile_factsheet_demo.json
```

### Composer 页面示例

```bash
ableppt render job_waterfall_demo.json
ableppt render job_scatter_demo.json
ableppt render job_bubble_demo.json
ableppt render job_range_snapshot_demo.json
ableppt render job_range_snapshot_sector_demo.json
```
...
`ableppt.chart_builder` 现在是兼容层，会把图表导入转发到 `ablechart`。
...
```bash
ableppt render job_gtm_demo.json     # 5 页演示：GDP/通胀/劳动力/估值/贸易
```
...
- **结构化输出**：`gtm_job_schema()` 返回 deck job 的 JSON Schema（内联图表 schema），
  做模型的约束解码；`ableppt validate-job job.json --deep` 深度校验闭环——
  加载真实数据源逐面板校验列名/类型（带 did-you-mean），错误喂回模型自修正
- **提示词工具包**：`ableppt describe-data <目录|job>` 生成数据列清单（列名/类型/样例/范围）；
  `ableppt prompt-kit <job|目录>` 一键组装 规则+页面示例+图表示例+数据清单 的完整提示词
- **原子安全渲染**：`ableppt render job.json --validate` 先深度校验，有错只输出报告不渲染——
...
- SDK loader：`from ableppt import load_jp_demo_chart_catalog`
...
2. 复杂图表能力应优先在 `ablechart` 中扩展，再接回 `ableppt`
3. `ableppt` 不应重新复制一套 chart engine
...
- CLI、SDK、layout、workflow、docs、samples → 改 `ableppt`
